...
final class DeviceConfigWriteGateTests: XCTestCase {
    ...
    // MARK: - Multi-character values (max_collection_backlog = "0.0")

    func testStringValueReadsMultiCharacterValues() {
        let rec = echoRecord(key: "max_collection_backlog", value: "0.0")
        let r = DeviceConfigReadProbe.ValueResponse(resultCode: 1, record: rec)
        // The single-byte reader loses the rest; the string reader is what a comparison must use.
        XCTAssertEqual(r.value(for: "max_collection_backlog"), 0x30)
        XCTAssertEqual(r.stringValue(for: "max_collection_backlog"), "0.0")
    }

    func testStringValueStopsAtNulSoEnvelopePaddingIsNotRead() {
        // The puffin envelope pads the inner payload to a 4-byte boundary; those NULs are not data.
        let rec = echoRecord(key: "enable_raw_data_w_ecg", value: "1") + [0, 0, 0]
        let r = DeviceConfigReadProbe.ValueResponse(resultCode: 1, record: rec)
        XCTAssertEqual(r.stringValue(for: "enable_raw_data_w_ecg"), "1")
    }

    func testStringValueClaimsNothingWhenThereIsNothingToClaim() {
        // Key not echoed at all.
        let other = DeviceConfigReadProbe.ValueResponse(resultCode: 1,
                                                        record: echoRecord(key: "enable_rfid", value: "0"))
        XCTAssertNil(other.stringValue(for: "enable_raw_data_w_ecg"))
        // Name field present but the record stops there — no value follows.
        let bare = DeviceConfigReadProbe.ValueResponse(
            resultCode: 1, record: [UInt8](repeating: 0, count: 32))
        XCTAssertNil(bare.stringValue(for: "enable_raw_data_w_ecg"))
        // A value slot that starts with NUL is empty, not "".
        let empty = DeviceConfigReadProbe.ValueResponse(
            resultCode: 1, record: echoRecord(key: "enable_raw_data_w_ecg", value: "") + [0])
        XCTAssertNil(empty.stringValue(for: "enable_raw_data_w_ecg"))
        // Non-ASCII is refused rather than transliterated.
        let binary = DeviceConfigReadProbe.ValueResponse(
            resultCode: 1, record: echoRecord(key: "enable_raw_data_w_ecg", value: "") + [0xFF, 0])
        XCTAssertNil(binary.stringValue(for: "enable_raw_data_w_ecg"))
    }
