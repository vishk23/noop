...
class PuffinExperiment(private val prefs: SharedPreferences) {
     ...
     *  `enable_raw_data_w_ecg` — the key the strap's own 115/116 enumeration listed, and which reads '0' on
     *  a subscription-free WHOOP MG whose three TOGGLE_LABRADOR commands all ack SUCCESS and emit nothing.
     *
     *  Its own key rather than a shared "ECG" one, because this repo gives every PERSISTENT STRAP WRITE its
     *  own deliberate opt-in ([isDeepDataEnabled] #174, [broadcastHr] #181) — reusing one switch for
     *  "listen for ECG packets" and "change a stored value on the strap" would let the second ride in on
     *  consent given for the first.
     *
     *  Reversible in one tap, default false, and additionally gated on `Whoop5Variant.isMG` at the call
     *  site — a plain 5.0 has no electrodes. Driven only by `WhoopBleClient.setEcgRawDataGate`, which always
     *  follows the write with a GET_DEVICE_CONFIG_VALUE(121) read-back. Mirrors the macOS
