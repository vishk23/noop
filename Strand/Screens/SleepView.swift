...
struct SleepView: View {
    ...
    /// defensible `personalSleepNeedHours`. Each night's credited sleep is the main night's
    /// `totalSleepMin` plus actual asleep minutes from separately-recorded naps (#1042). The
    /// recency-weighted, capped, asymmetric running balance (recent nights weigh more, |balance|
    /// bounded, oversleep repays less than a short night accrues) lives in `SleepDebt.ledger`;
    /// here we only supply the series + need. Skips nights with no sleep (the analytics function
    /// does the skip). (#242)
    ...
    /// APPROXIMATE per-night need − (main sleep + nap sleep), floored at 0 (no "credit").
    /// Measured against the SAME defensible `personalSleepNeedHours` the ledger card uses, so
    /// this per-night tile and the running-balance card agree on what "need" means — one need
    /// across both debt surfaces, rather than the self-referential `sleepNeedMin` mean this
    /// used to use. (#242)
