...
object IntelligenceEngine {
    ...
    /**
     * The night window's skin-temperature rows plus BOTH SpO2 streams for one owner, in one place.
     *
     * Extracted from `analyzeRecentOnCpu` purely for bytecode headroom (#1524); behaviour is unchanged.
     * - `skin`: raw skin-temp register samples (#938 converts them per family at the call site).
     * - `spo2`: #93 WHOOP 4.0 raw red/IR PPG samples; analyzeDay banks the nightly ADC means. Empty on a
     *   5/MG (no v24 spo2 channels) → the raw means stay null.
     * - `spo2Pct`: the 5/MG's own per-second `@82` percentages (v34 / MIGRATION_33_34), read from the
     *   DURABLE table that is never pruned — re-scoring an old night still finds its readings, where the
     *   capped aux table they were forked from holds only ~7 days. Empty on a WHOOP 4.0 and on windows
     *   offloaded before MIGRATION_33_34 → the percentage stays null rather than fabricated.
     */
    private suspend fun nightSkinAndSpo2Streams(
        repo: WhoopRepository,
        owner: String,
        from: Long,
        to: Long,
    ) = Triple(
        repo.skinTempSamples(owner, from, to, STREAM_LIMIT),
        repo.spo2Samples(owner, from, to, STREAM_LIMIT),
        repo.spo2PctSamples(owner, from, to, STREAM_LIMIT),
    )
    ...
    private suspend fun analyzeRecentOnCpu(
        ...
        for (offset in 0 until maxDays) {
            ...
            // Skin temp + both SpO2 streams, read via [nightSkinAndSpo2Streams] rather than as three
            // inline suspend calls: `analyzeRecentOnCpu` sits near the JaCoCo-instrumented method-size
            // budget (#1524), and each suspension point in this method costs instrumented bytes. Same
            // reads, same order, same limits — extracted purely for bytecode headroom, like
            // [spo2CandidateMean] and [persistFitnessVitalityAndSteps].
            val (skin, spo2, spo2Pct) = nightSkinAndSpo2Streams(repo, owner, from, to)
