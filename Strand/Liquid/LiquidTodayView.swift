...
struct LiquidTodayView: View {
    ...
    private func ktileFor(_ metric: KeyMetric, hrv: Double?, rhr: Double?) -> some View {
        ...
        case .bloodOxygen:
            ...
            // Queue 11a: when every REAL source is nil, falls back to the device-conditional
            // "spo2_candidate" mean (WHOOP: `spo2_candidate_82`; Oura: ceiling@100 `0x6F`, see
            // `AnalyticsEngine.nightlySpo2CeilingMean`) only while the experimental toggle is ON — same
            // gating as the classic tile, never as the default.
            let spo2Real = Self.perFieldVital(today: displayDay?.spo2Pct, vitalsCarry: vitalsDay?.spo2Pct, fieldCarry: spo2Day?.spo2Pct)
...
private struct LiquidBatteryButton: View {
    ...
    private var batteryDisplay: LiquidTodayView.StrapBatteryDisplay {
        ...
        return .resolve(
            ...
            // `chargingEffective`, not the raw flag — the 5.0 charge bit is unverified and was observed
            // reading 0 on a charging strap; a rising SoC is the witness that can't lie. See
            // StrapChargeInference.
            charging: live.chargingEffective
        )
    }
...
/// Kept here in the Data Sources card as the detailed view; `LiquidBatteryButton` above is the header's
/// ambient at-a-glance signal.
...
private struct LiquidStrapBatteryRow: View {
    ...
    private func batteryText(pct: Double) -> String {
        ...
        if live.chargingEffective == true { return "\(base) · \(String(localized: "Charging"))" }
